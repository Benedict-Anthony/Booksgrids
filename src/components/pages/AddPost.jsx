...
function AddPost() {
    ...
    const handleCaptionChange = (e) => {
        ...
        if (captions.includes(+e.currentTarget.value)) {
            ...
            setCaption([...captions, +e.currentTarget.value].map((caps) => parseInt(caps)))
        }
    }
    ...
    async function submitValidatedPost() {
        // setCaption(captions.map((caps) => parseInt(caps)))
        ...
        try {
            const response = await AxiosUserInstance.post("create/", formData)
    ...
    function submitPost(e) {
        ...
        if (title === "" || excerpt === "" || content === "") {
            ...
            setError(true)
            setMessage("Please provide a descriptive image for your book")
        }
        else if (captions.length === 0 || captions.length >= 2) {
            setError(true)
            setMessage("Please select one caption for your Book")
        }
