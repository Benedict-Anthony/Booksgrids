...
function UpdateProfile() {
    ...
    async function submitData() {
        ...
        if (avata) { formData.append("image", avata[0]) }
        formData.append("user", accessToken().user_id)

        const response = await AxiosUserInstance.put("user/profile-create/", formData)
    ...
    const fetchOldData = async () => {
        const response = await AxiosUserInstance.get("user/profile-create/")
        const { data } = response
        SetProfileData({
            ...profileData,
    ...
    return (
        <div>
            {isProcessing ? <Processing /> : (
                <Form formText={"Update your profile"} handleSubmit={submitProfile} image={profile} buttonText={"Update Profile"}>
